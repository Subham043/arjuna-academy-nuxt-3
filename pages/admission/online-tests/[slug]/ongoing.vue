...
}>(() => API_ROUTES.admission_registration_test + `/${route.params.slug}/question-set`, {
  key: 'admission_online_test_question_set_' + route.params.slug,
  lazy: true
})
...
watch(() => data.value, async (value) => {
  if (value) {
    ...
    if (value.question_set.test_status === 'Completed') {
      await exit()
      router.push(`/admission/online-tests/${route.params.slug}/report`)
    ...
    if (value.question_set.test_status === 'Eliminated') {
      ...
      router.push(`/admission/online-tests/${route.params.slug}`)
    }
  }
})
...
const fillAnswerHandler = async () => {
  ...
  try {
    ...
    const response = await useCustomFetch(API_ROUTES.admission_registration_test + `/${route.params.slug}/fill-answer`, {
      method: 'POST',
      body: formData
    })
...
const eliminatedHandler = async (reason: string) => {
  ...
  try {
    ...
    const response = await useCustomFetch(API_ROUTES.admission_registration_test + `/${route.params.slug}/eliminated`, {
      method: 'POST',
      body: formData
    })
    const content = await response.json()
